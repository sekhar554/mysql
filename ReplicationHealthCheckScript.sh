...
### VARIABLES ### \
EMAIL="rajasekhar.s@infinitylabs.in"
SERVER=$(Infinitylabs)
USER='root'
PASSWORD='root'
MYSQL_CHECK=$(mysql $USER $PASSWORD -e "SHOW VARIABLES LIKE '%version%';" || echo 1)
LAST_ERRNO=$(mysql $USER $PASSWORD -e "SHOW SLAVE STATUS\G" | grep "Last_Errno" | awk '{ print $2 }')
SECONDS_BEHIND_MASTER=$(mysql $USER $PASSWORD -e "SHOW SLAVE STATUS\G"| grep "Seconds_Behind_Master" | awk '{ print $2 }')
IO_IS_RUNNING=$(mysql $USER $PASSWORD -e "SHOW SLAVE STATUS\G" | grep "Slave_IO_Running" | awk '{ print $2 }')
SQL_IS_RUNNING=$(mysql $USER $PASSWORD -e "SHOW SLAVE STATUS\G" | grep "Slave_SQL_Running" | awk '{ print $2 }')
ERRORS=()
